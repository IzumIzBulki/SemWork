...
# ; строки и столбца. Аргументы num_rows и num_columns указывают число строк и столбцов таблицы, 
...
string = int(input(f'Введите количество строк: '))
column = int(input(f'Введите количество столбцов: '))

def print_operation_table(vertical_length, horizontal_length):
    for i in range(1, horizontal_length+1):
        number=0
        for _ in range(vertical_length):
            number=number+i
            print(number, end=" "'\t')
        print()

print_operation_table(column, string)
